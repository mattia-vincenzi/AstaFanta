...
import { allowedBudgetRoles, assignPlayer, createInitialState, teamSummary } from './domain.js';
...
function strategy() {
  const summary = teamSummary(state, state.ownTeamId);
  return `<section class="panel narrow"><h2>Strategia</h2><p class="muted">Questi valori generano avvisi, ma non bloccano l'asta.</p><form id="strategy-form"><label>La mia squadra<select name="ownTeamId">${state.teams.map((t) => `<option value="${t.id}" ${t.id === state.ownTeamId ? 'selected' : ''}>${escape(t.name)}</option>`).join('')}</select></label><label>Nome squadra<input name="teamName" value="${escape(ownTeam().name)}"></label><label>Budget iniziale<input name="budget" type="number" min="1" value="${ownTeam().budget}"></label><label>Dimensione rosa<input name="rosterSize" type="number" min="1" value="${ownTeam().rosterSize}"></label><button>Salva configurazione</button></form><hr><h3>Situazione attuale</h3><p>Spesi ${summary.spent} · Residuo ${summary.remaining} · Slot ${summary.slotsRemaining}</p></section>`;
}
