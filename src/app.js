...
import { addStrategyRole, assignPlayer, createSetup, opponentSummaries, ownRoleSummaries, removeStrategyRole, renameTeam, resizeLeague, roleSummaries, setupRules, strategyWarnings, teamSummary, validatePlayer } from './domain.js';
...
function dashboard() {
  ...
  return `<section class="command-center ${state.setup}"><div class="hero-kpi"><span>Crediti disponibili</span><strong>${mine.remaining}</strong><small>${mine.spent} spesi · ${mine.slotsRemaining} slot liberi</small></div><div class="role-overview"><h2>${state.setup === 'classic' ? 'La mia rosa per reparto' : 'La mia spesa per ruolo'}</h2><div class="role-cards">${roleCards.map((item) => `<article class="role-card ${item.complete ? 'complete' : ''} ${item.budgetStatus ? `budget-${item.budgetStatus}` : ''}">${roleBadge(item.role)}<strong>${item.value}</strong><div class="meter" aria-label="${item.progress}%"><i style="width:${item.progress}%"></i></div><span>${item.detail}</span>${item.supporting ? `<small>${item.supporting}</small>` : ''}</article>`).join('')}</div></div><div class="opponents"><h2>Altre squadre</h2><div class="table-wrap compact"><table><thead><tr><th>${sortable('opponents', 'name', 'Squadra')}</th><th>${sortable('opponents', 'remaining', 'Residuo')}</th><th>${sortable('opponents', 'spent', 'Spesi')}</th><th>${sortable('opponents', 'players', 'Rosa')}</th>${state.setup === 'classic' ? '<th>Reparti</th>' : ''}</tr></thead><tbody>${opponents.map((team) => `<tr><td>${escape(team.name)}</td><td><b>${team.remaining}</b></td><td>${team.spent}</td><td>${team.players}/${team.rosterSize}</td>${state.setup === 'classic' ? `<td class="role-counts">${roleSummaries(state, team.id).map((item) => `<span class="${item.complete ? 'complete' : ''}">${item.role} ${item.players}/${item.slots}</span>`).join('')}</td>` : ''}</tr>`).join('')}</tbody></table></div></div></section>`;
}
...
function targetSections(roles) {
  ...
  return `<div class="strategy-targets"><div class="section-title"><div><h3>Giocatori target per fascia</h3><p class="muted">Crea fasce di spesa personalizzate e annota un giocatore per riga.</p></div></div>${roles.map((role) => {
    ...
    return `<section class="target-role-section"><div class="target-role-heading">${roleBadge(role)}<button type="button" data-action="add-target-band" data-role="${role}">Aggiungi fascia</button></div>${bands.length ? `<div class="target-bands">${bands.map((band, index) => `<article class="target-band"><div class="target-band-fields"><label>Nome fascia<input name="target-${role}-${index}-label" value="${escape(band.label)}" placeholder="es. Low cost"></label><label>Da crediti<input name="target-${role}-${index}-min" type="number" min="0" value="${band.min}"></label><label>A crediti<input name="target-${role}-${index}-max" type="number" min="0" value="${band.max}"></label></div><label>Giocatori target<textarea name="target-${role}-${index}-players" rows="3" placeholder="Un giocatore per riga">${escape(band.players)}</textarea></label><button type="button" class="remove-player" data-action="remove-target-band" data-role="${role}" data-index="${index}">Rimuovi fascia</button></article>`).join('')}</div>` : '<p class="muted target-empty">Nessuna fascia configurata.</p>'}</section>`;
  }).join('')}</div>`;
}
...
root.addEventListener('submit', (event) => {
  ...
  try {
    ...
    if (formId === 'strategy-form') { const ownTeamId = form.get('ownTeamId'); const current = state.teams.find((team) => team.id === ownTeamId); const updatedTeam = { ...current, name: form.get('teamName').trim() || current.name, budget: Number(form.get('budget')), rosterSize: state.setup === 'classic' ? 25 : Number(form.get('rosterSize')) }; const fields = state.setup === 'classic' ? ['min', 'target', 'max'] : ['slots', 'min', 'target', 'max']; const roleBudgets = Object.fromEntries(Object.keys(state.strategy.roleBudgets).map((role) => [role, { ...state.strategy.roleBudgets[role], ...Object.fromEntries(fields.map((field) => [field, Number(form.get(`${role}-${field}`))])) }])); const roleTargets = Object.fromEntries(Object.keys(state.strategy.roleBudgets).map((role) => [role, (state.strategy.roleTargets?.[role] || []).map((band, index) => ({ ...band, label: String(form.get(`target-${role}-${index}-label`) || '').trim() || 'Fascia target', min: Number(form.get(`target-${role}-${index}-min`)) || 0, max: Number(form.get(`target-${role}-${index}-max`)) || 0, players: String(form.get(`target-${role}-${index}-players`) || '') }))])); const strategy = { ...state.strategy, roleBudgets, roleTargets }; state = { ...state, ownTeamId, teams: state.teams.map((team) => team.id === ownTeamId ? updatedTeam : team), strategy }; persist(); render(); return; }
