...
let editingPlayerId = '';
let tableSort = {
  opponents: { key: 'remaining', direction: 'desc' },
  auction: { key: 'name', direction: 'asc' },
  strategy: { key: null, direction: 'asc' },
  catalogue: { key: 'name', direction: 'asc' },
};
let feedback = '';
...
const sortableHeader = (table, key, label, type = 'text') => {
  ...
  return `<th class="sortable-header" aria-sort="${ariaSort}"><button type="button" class="sort" data-sort-table="${table}" data-sort-key="${key}" data-sort-type="${type}"><span>${label}</span><span class="sort-indicator" aria-hidden="true">${indicator}</span></button></th>`;
};
...
function sortRenderedTable(button, direction) {
  const table = button.closest('table');
  const body = table?.tBodies[0];
  const columnIndex = button.closest('th')?.cellIndex;
  if (!body || columnIndex === undefined) return;

  const rows = [...body.rows];
  const valueFor = (row) => {
    const cell = row.cells[columnIndex];
    const control = cell?.querySelector('input, select');
    return (control?.value ?? cell?.dataset.sortValue ?? cell?.textContent ?? '').trim();
  };
  const compare = (left, right) => {
    const a = valueFor(left.row);
    const b = valueFor(right.row);
    let result;
    if (button.dataset.sortType === 'number') {
      const aNumber = Number(a);
      const bNumber = Number(b);
      result = (Number.isFinite(aNumber) ? aNumber : Number.POSITIVE_INFINITY)
        - (Number.isFinite(bNumber) ? bNumber : Number.POSITIVE_INFINITY);
    } else {
      result = a.localeCompare(b, 'it', { numeric: true, sensitivity: 'base' });
    }
    return (direction === 'desc' ? -result : result) || left.index - right.index;
  };

  rows.map((row, index) => ({ row, index })).sort(compare).forEach(({ row }) => body.append(row));
...
function header() {
  ...
  return `<header class="app-header"><div><p class="mode-label">Modalità ${rules().label}</p><h1>Asta ${rules().label}</h1><p>${escape(ownTeam().name)} · ${summary.remaining} crediti · ${summary.slotsRemaining} slot</p></div><p class="sr-only" aria-live="polite">${escape(feedback)}</p><div class="backup"><button type="button" data-action="export">Esporta backup</button><label class="button">Importa backup<input data-action="import" type="file" accept="application/json" hidden></label><button type="button" data-action="reset" class="quiet">Nuova asta</button></div></header>`;
}

function navigation() {
  return `<nav aria-label="Sezioni principali">${[['asta', 'Asta live'], ['squadre', 'Squadre'], ['strategia', 'Strategia'], ['catalogo', 'Catalogo']].map(([id, label]) => `<button type="button" class="${tab === id ? 'active' : ''}" data-tab="${id}" aria-current="${tab === id ? 'page' : 'false'}">${label}</button>`).join('')}</nav>`;
}
...
function dashboard() {
  ...
  const opponents = sortRows(opponentSummaries(state).map((team) => ({
    ...team,
    departments: roleSummaries(state, team.id).map((item) => `${item.role} ${item.players}/${item.slots}`).join(' '),
  })), tableSort.opponents.key, tableSort.opponents.direction);
  return `<section class="command-center ${state.setup}"><div class="hero-kpi"><span>Crediti disponibili</span><strong>${mine.remaining}</strong><small>${mine.spent} spesi · ${mine.slotsRemaining} slot liberi</small></div><div class="role-overview"><h2>${state.setup === 'classic' ? 'La mia rosa per reparto' : 'La mia spesa per ruolo'}</h2><div class="role-cards">${roleCards.map((item) => `<article class="role-card ${item.complete ? 'complete' : ''} ${item.budgetStatus ? `budget-${item.budgetStatus}` : ''}">${roleBadge(item.role)}<strong>${item.value}</strong><div class="meter" aria-label="${item.progress}%"><i style="width:${item.progress}%"></i></div><span>${item.detail}</span>${item.supporting ? `<small>${item.supporting}</small>` : ''}</article>`).join('')}</div></div><div class="opponents"><h2>Tutte le squadre</h2><div class="table-wrap compact"><table><thead><tr>${sortableHeader('opponents', 'name', 'Squadra')}${sortableHeader('opponents', 'remaining', 'Residuo', 'number')}${sortableHeader('opponents', 'spent', 'Spesi', 'number')}${sortableHeader('opponents', 'players', 'Rosa', 'number')}${state.setup === 'classic' ? sortableHeader('opponents', 'departments', 'Reparti') : ''}</tr></thead><tbody>${opponents.map((team) => `<tr class="${team.id === state.ownTeamId ? 'own-team-row' : ''}"><td>${escape(team.name)}${team.id === state.ownTeamId ? ' <span class="own-team-label">La mia squadra</span>' : ''}</td><td><b>${team.remaining}</b></td><td>${team.spent}</td><td>${team.players}/${team.rosterSize}</td>${state.setup === 'classic' ? `<td class="role-counts" data-sort-value="${escape(team.departments)}">${roleSummaries(state, team.id).map((item) => `<span class="${item.complete ? 'complete' : ''}">${item.role} ${item.players}/${item.slots}</span>`).join('')}</td>` : ''}</tr>`).join('')}</tbody></table></div></div></section>`;
}

function auction() {
  const columns = catalogueColumns(state.setup);
  const available = filterPlayers(state.players, assignedIds(), filters, state.setup);
  const visible = sortRows(available, tableSort.auction.key, tableSort.auction.direction).slice(0, 120);
...
function sortedStrategyRows(budgets) {
  const rows = Object.entries(budgets).map(([role, value]) => ({ role, ...value }));
  return tableSort.strategy.key ? sortRows(rows, tableSort.strategy.key, tableSort.strategy.direction) : rows;
}
...
function strategy() {
  ...
  return `<section class="panel strategy-panel"><div><p class="mode-label">Solo la mia squadra</p><h2>Strategia Mantra</h2><p class="muted">Scegli i ruoli da pianificare e imposta slot e soglie di spesa. Gli avvisi non bloccano l'asta.</p></div><form id="strategy-form" novalidate><div class="strategy-top"><label>La mia squadra<select name="ownTeamId">${state.teams.map((team) => `<option value="${team.id}" ${team.id === state.ownTeamId ? 'selected' : ''}>${escape(team.name)}</option>`).join('')}</select></label><label>Nome squadra<input name="teamName" value="${escape(ownTeam().name)}"></label><label>Budget iniziale<input name="budget" type="number" min="1" value="${ownTeam().budget}"></label><label>Dimensione rosa<input name="rosterSize" type="number" min="1" value="${ownTeam().rosterSize}"></label></div><div class="strategy-heading"><h3>Budget per ruolo</h3>${missingRoles.length ? `<div class="strategy-add-role"><label>Aggiungi ruolo<select data-strategy-role>${missingRoles.map((role) => `<option>${role}</option>`).join('')}</select></label><button type="button" data-action="add-strategy-role">Aggiungi</button></div>` : '<span class="muted">Tutti i ruoli sono attivi</span>'}</div><div class="table-wrap"><table><thead><tr>${sortableHeader('strategy', 'role', 'Ruolo')}${sortableHeader('strategy', 'slots', 'Slot', 'number')}${sortableHeader('strategy', 'min', 'Minimo', 'number')}${sortableHeader('strategy', 'target', 'Obiettivo', 'number')}${sortableHeader('strategy', 'max', 'Massimo', 'number')}<th><span class="sr-only">Azioni</span></th></tr></thead><tbody>${sortedStrategyRows(budgets).map(({ role, ...value }) => `<tr><td>${roleBadge(role)}</td>${['slots', 'min', 'target', 'max'].map((field) => `<td><input name="${role}-${field}" type="number" min="0" value="${value[field]}"></td>`).join('')}<td><button type="button" data-action="remove-strategy-role" data-role="${role}" class="remove-player">Rimuovi</button></td></tr>`).join('')}</tbody></table></div>${targetSections(Object.keys(budgets))}<button type="submit">Salva configurazione</button></form>${warnings.length ? `<div class="warnings" role="alert">${warnings.map((warning) => `${warning.role} supera il massimo (${warning.spent}/${warning.limit}).`).join('<br>')}</div>` : ''}<div class="strategy-summary">Spesi <b>${summary.spent}</b> · Residuo <b>${summary.remaining}</b> · Slot liberi <b>${summary.slotsRemaining}</b></div></section>`;
}

function catalogue() {
  const sorted = sortRows(state.players, tableSort.catalogue.key, tableSort.catalogue.direction);
  const columns = catalogueColumns(state.setup);
  return `<section class="panel"><div class="section-heading"><div><p class="mode-label">Catalogo ${rules().label}</p><h2>Listone modificabile</h2><p class="muted">Aggiungi o correggi giocatori. Gli assegnati non possono essere eliminati.</p></div><button type="button" data-action="new-player">Aggiungi giocatore</button></div><form id="player-form" class="player-form" hidden novalidate><label>ID univoco<input name="id" required></label><label>Nome<input name="name" required></label><label>Squadra<input name="team" required></label><label>Ruoli<input name="roles" placeholder="${state.setup === 'classic' ? 'P, D, C o A' : 'es. M/C'}" required></label><label>Qt<input name="qt" type="number" value="0"></label><label>${columns.fvm}<input name="fvm" type="number" value="0"></label><button type="submit">Salva giocatore</button></form><div class="table-wrap"><table><thead><tr>${sortableHeader('catalogue', 'id', 'ID')}${sortableHeader('catalogue', 'name', 'Nome')}${sortableHeader('catalogue', 'team', 'Squadra')}${sortableHeader('catalogue', 'roles', columns.role)}${sortableHeader('catalogue', 'qt', 'Qt', 'number')}${sortableHeader('catalogue', 'fvm', columns.fvm, 'number')}<th>Azioni</th></tr></thead><tbody>${sorted.map((player) => `<tr><td>${escape(player.id)}</td><td><b>${escape(player.name)}</b></td><td>${escape(player.team)}</td><td data-sort-value="${escape(player.roles.join('/'))}">${player.roles.map(roleBadge).join('')}</td><td>${player.qt}</td><td>${player.fvm}</td><td><button type="button" data-action="edit-player" data-id="${player.id}" class="quiet small">Modifica</button>${assignedIds().has(player.id) ? '' : ` <button type="button" data-action="delete-player" data-id="${player.id}" class="danger">Elimina</button>`}</td></tr>`).join('')}</tbody></table></div></section>`;
}
...
function renderCatalogueLoadError() {
  ...
  root.innerHTML = '<main class="setup-screen"><section class="panel load-error" role="alert"><h1>Impossibile caricare i cataloghi.</h1><p>Controlla la connessione e riprova.</p><button type="button" data-action="retry-catalogues">Riprova</button></section></main>';
}
...
root.addEventListener('click', (event) => {
  ...
  if (sortButton) { const current = tableSort[sortButton.dataset.sortTable]; const direction = current.key === sortButton.dataset.sortKey && current.direction === 'asc' ? 'desc' : 'asc'; tableSort = { ...tableSort, [sortButton.dataset.sortTable]: { key: sortButton.dataset.sortKey, direction } }; sortRenderedTable(sortButton, direction); return; }
  ...
  if (target.dataset.action === 'undo') { state = { ...state, assignments: state.assignments.slice(0, -1) }; persist(); render(); showNotification('success', 'Ultimo acquisto annullato'); return; }
  if (target.dataset.action === 'remove') { state = { ...state, assignments: state.assignments.filter((assignment) => assignment.playerId !== target.dataset.id) }; persist(); render(); showNotification('success', 'Giocatore rimosso'); return; }
  if (target.dataset.action === 'add-target-band') { const role = target.dataset.role; const roleTargets = { ...(state.strategy.roleTargets || {}) }; roleTargets[role] = [...(roleTargets[role] || []), { id: `${role}-${Date.now()}`, label: 'Nuova fascia', min: 0, max: 0, players: '' }]; state = { ...state, strategy: { ...state.strategy, roleTargets } }; persist(); render(); showNotification('success', 'Fascia target aggiunta'); return; }
  if (target.dataset.action === 'remove-target-band') { const role = target.dataset.role; const index = Number(target.dataset.index); const roleTargets = { ...(state.strategy.roleTargets || {}) }; roleTargets[role] = (roleTargets[role] || []).filter((_, bandIndex) => bandIndex !== index); state = { ...state, strategy: { ...state.strategy, roleTargets } }; persist(); render(); showNotification('success', 'Fascia target rimossa'); return; }
  if (target.dataset.action === 'remove-strategy-role') { state = removeStrategyRole(state, target.dataset.role); persist(); render(); showNotification('success', 'Ruolo rimosso dalla strategia'); return; }
  if (target.dataset.action === 'add-strategy-role') { state = addStrategyRole(state, root.querySelector('[data-strategy-role]').value); persist(); render(); showNotification('success', 'Ruolo aggiunto alla strategia'); return; }
  if (target.dataset.action === 'reset') { if (confirm('Creare una nuova asta? I dati correnti verranno rimossi dal browser.')) { dismissNotification(); state = null; localStorage.removeItem('tool-asta-mantra-state-v1'); tab = 'asta'; render(); } return; }
  if (target.dataset.action === 'export') { const link = document.createElement('a'); link.href = URL.createObjectURL(new Blob([exportState(state)], { type: 'application/json' })); link.download = `asta-${state.setup}-backup.json`; link.click(); URL.revokeObjectURL(link.href); showNotification('info', 'Backup esportato'); return; }
  if (target.dataset.action === 'new-player') { editingPlayerId = ''; root.querySelector('#player-form').hidden = false; return; }
  if (target.dataset.action === 'delete-player' && confirm('Eliminare questo giocatore?')) { state = { ...state, players: state.players.filter((player) => player.id !== target.dataset.id) }; persist(); render(); showNotification('success', 'Giocatore eliminato'); return; }
...
root.addEventListener('change', async (event) => {
  ...
  if (event.target.name === 'ownTeamId' && event.target.closest('#strategy-form')) { state = { ...state, ownTeamId: event.target.value }; persist(); render(); showNotification('success', 'Squadra personale aggiornata'); return; }
  if (event.target.dataset.action === 'import') { try { state = importState(await event.target.files[0].text()); selectedPlayerId = ''; selectedTeamId = ''; persist(); render(); showNotification('success', 'Backup importato'); } catch { event.target.value = ''; showNotification('error', 'Backup non valido'); } }
});
