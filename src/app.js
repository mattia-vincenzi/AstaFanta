...
root.addEventListener('submit', (event) => {
  ...
  try {
    ...
    if (event.target.id === 'strategy-form') { const ownTeamId = form.get('ownTeamId'); const current = state.teams.find((team) => team.id === ownTeamId); const updatedTeam = { ...current, name: form.get('teamName').trim() || current.name, budget: Number(form.get('budget')), rosterSize: state.setup === 'classic' ? 25 : Number(form.get('rosterSize')) }; const fields = state.setup === 'classic' ? ['min', 'target', 'max'] : ['slots', 'min', 'target', 'max']; const roleBudgets = Object.fromEntries(Object.keys(state.strategy.roleBudgets).map((role) => [role, { ...state.strategy.roleBudgets[role], ...Object.fromEntries(fields.map((field) => [field, Number(form.get(`${role}-${field}`))])) }])); const strategy = { ...state.strategy, roleBudgets }; state = { ...state, ownTeamId, teams: state.teams.map((team) => team.id === ownTeamId ? updatedTeam : team), strategy }; persist(); render(); return; }
