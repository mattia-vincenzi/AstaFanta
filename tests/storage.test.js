...
test('state survives JSON export and import', () => {
  ...
  assert.deepEqual(importState(exportState(state)), { ...state, setup: 'mantra', classicSlots: null });
});
...
test('save writes a reloadable state', () => {
  ...
  assert.deepEqual(loadState(storage, {}), { ...state, setup: 'mantra', classicSlots: null });
});
